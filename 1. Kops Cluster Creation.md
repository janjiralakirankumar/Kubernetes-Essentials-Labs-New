...
* In security groups, include ports `22 (SSH),` `80 (HTTP)` and 443 (HTTPs).
...
#### Note:
* Replace the `nodes-us-east-2a.kiran-2024-03-19-16-03.k8s.local` with the `auto-scaling group (master or worker) and also replace `region` with your region.`
* Make sure the `Max capacity` of the `Auto Scaling Group` is `above or equal` to the number you are trying to scale to.

1. To Update Master Node/Instance
```
...
```
2. To Update Slave Nodes/Instance
```
...
```
##### If you are unable to retrieve the data of nodes or the cluster, you can use the following command. This command is particularly useful if you have downscaled or scaled it up again:
```
